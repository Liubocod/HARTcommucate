...
* 返回值说明: void
*
* 功能介绍: I2C初始化，将P0.4(SCK)、P0.5(SDA)设置为普通输出模式，并置低电平
*
* 关键备注：
*
* 未来提高:
...
{	
       ...
       {		
           ...
           if ( ((WriteAddr & 0x07) == 0) && (WrittedByteNum < NumByteToWrite) ) //The 2K EEPROM is capable of 8-byte Page Writes
           {
                 ...
                 //IICRomDelay(IICROM_DELAY_4MS); // 目前当一次写127个字节时（该芯片可写入字节数为128），至少需要4ms，	   
                 Start_IICROM();                          // Send START condition 	 
                 IIROMSendByte(IICROM_DEVICE_ADDRESS);   // Send EEPROM address for write
                 if (G_IICROMDeviceAck == FALSE)
                 {
...
/**void  IICROMReadAndWriteTest()//函数名称和参量和返回值类型。
* 变量范围: 无 
*
* 返回值说明: 
*
...
//	TEST_TIME_PIN_OUT_H;  // 两者之间不加延时时，高电平持续时间约0.8us
